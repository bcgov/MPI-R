...
#This script takes data from the file mpi_shortraw.rds, creates plots and then saves them as nested
#dataframes (Rds binaries)
#libraries-----
...
by_region_plots <- short%>%
  group_by(region)%>%
  nest()%>%
  mutate(`5. Status` = map(data, plot_agg_val_by_status),
         `6. Status Last 4 Quarters` = map(data, lfq_plot, project_status),
  ...
  arrange(region)%>%
  ungroup()
 saveRDS(all_regions_plots, here::here("processed_data", "all_regions_plots.rds"))
 saveRDS(by_region_plots, here::here("processed_data", "by_region_plots.rds"))
