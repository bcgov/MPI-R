...
lfq_table <- function(df, var){
  ...
  df <- df%>%
    mutate(change_from_previous_quarter = percent(.data[[lq]]/.data[[slq]]-1),
           change_from_previous_year = percent(.data[[lq]]/.data[[ly]]-1))
}
...
category_breakdown <- function(df){
  ...
  output_df <- bind_rows(by_both, by_stage, by_status)%>%
    mutate(percentage_of_total = percent(estimated_capital_cost_M/total))
...
input02.6 <- function(df){
  ...
  most_recent%>%
    ...
    inner_join(year_ago)%>%
    mutate(change_from_previous_quarter = percent(Total/total_last_quarter-1),
           change_from_previous_year = percent(Total/total_year_ago-1))%>%
