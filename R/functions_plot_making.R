...
lfq_plot <- function(df, var){
  ...
  plt <- ggplot(df,
                aes(quarter,
                    ...
                    text = paste(
                      "\nIn", yearquarter(quarter),
                      "total project costs was", comma(`Estimated Costs (M)`), "Million",
...
plot02.1b <- function(df){
  ...
  plt <- ggplot(df,
                aes(quarter,
                    ...
                    text = paste(
                      "\nIn", yearquarter(quarter),"total project costs for\n", str_to_lower(project_category_name),   
                      "was", comma(`Estimated Costs (M)`), "Million.")))+
    geom_area()+
    scale_y_continuous(labels = comma)+
    scale_fill_viridis_d()+
...
plot_agg_val_by_status <- function(df){
  df <- df%>%
    ...
    summarize(`Estimated Costs (M)` = sum(estimated_cost, na.rm=TRUE))%>%
    as_tsibble(key=project_status, index=quarter)%>%
    fill_gaps(`Estimated Costs (M)`=0)%>%
    mutate(quarter=yq(quarter))
...
plt <- ggplot(last_quarter, aes(x = estimated_cost,
             ...
             text = paste(
               ...
               "\nwith an estimated cost of", comma(estimated_cost), "Million.")))+
    geom_point(alpha=.25)+
